// 汐见地图 v1.2 · 高精度底图 / 单 Canvas Overlay / 最大缩放限制
...
(() => {
  ...
  class MapRepository {
    ...
    async load(force = false) {
      ...
      let baseImageUrl = this.data.canvas.baseImage;
      const assets = hostWin.ShiomiAssets || localWin.ShiomiAssets;
      if (assets && typeof assets.resolve === 'function' && assets.isConfigured?.()) {
        try {
          const remoteUrl = await assets.resolve(this.data.canvas.baseImageAsset || 'map.city.base');
          if (remoteUrl) baseImageUrl = remoteUrl;
        } catch (error) {
          console.warn(LOG, '远程底图解析失败，回退到角色卡内底图。', error);
        }
      }
  ...
  class MapRenderer {
    ...
    syncBase() {
      ...
      this.base.style.width = `${this.data.canvas.width}px`;
      this.base.style.height = `${this.data.canvas.height}px`;
      this.base.style.transform = `translate3d(${this.viewport.x}px,${this.viewport.y}px,0) scale(${s})`;
    }
    ...
    drawSymbol(ctx, node, x, y, active = false, current = false) {
      ...
      try {
        const path = new hostWin.Path2D(ICON_PATHS[this.categoryKey(node)]);
  ...
  class MapUI {
    ...
    styles() {
      return `
        ...
        .shiomi-map-launch{position:fixed;left:max(12px,env(safe-area-inset-left));bottom:max(74px,calc(env(safe-area-inset-bottom) + 62px));z-index:2147482500;display:flex;align-items:center;gap:7px;border:1px solid rgba(54,50,45,.16);border-radius:12px;padding:10px 13px;background:rgba(252,249,242,.95);color:#383630;font:600 13px/1.2 "Noto Sans CJK SC","Microsoft YaHei",sans-serif;box-shadow:0 4px 16px rgba(35,30,25,.16);cursor:pointer;touch-action:manipulation}
  ...
  class MapLifecycle {
    constructor(){
      ...
      this.locationDirty=true;this.currentLocationText='';this.currentNode=null;this.disposers=[];this.pointers=new Map();this.gesture=null;this.dragMoved=false;
    }
    ...
    bindMapInput(){
      ...
      const down=event=>{if(event.pointerType==='mouse'&&event.button!==0)return;canvas.setPointerCapture?.(event.pointerId);const p=point(event);this.pointers.set(event.pointerId,p);this.dragMoved=false;if(this.pointers.size===1)this.gesture={type:'pan',last:p,start:p};else if(this.pointers.size===2){const[a,b]=[...this.pointers.values()];this.gesture={type:'pinch',distance:Math.hypot(a.x-b.x,a.y-b.y),zoom:this.viewport.zoom};}canvas.classList.add('dragging');this.ui.fadeHint();};
      ...
      const wheel=event=>{event.preventDefault();this.ui.fadeHint();const p=point(event);this.viewport.setZoom(this.viewport.zoom*Math.exp(-event.deltaY*.00135),p.x,p.y);this.requestRender();};
    ...
    close(){if(this.destroyed)return;this.isOpen=false;this.ui.setOpen(false);this.ui.hideResults();this.ui.setRegions(false);this.pointers.clear();this.gesture=null;console.info(LOG,'地图已关闭；没有后台渲染循环。');}
